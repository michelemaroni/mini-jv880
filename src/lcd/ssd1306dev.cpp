...
constexpr auto FontSingle = Font<FONT_SIZE, decltype(SingleColumn)>(Font5x8, SingleColumn);
constexpr auto FontDouble = Font<FONT_SIZE, decltype(DoubleColumn)>(Font5x8, DoubleColumn);


CSSD1306Dev::CSSD1306Dev (unsigned nWidth, unsigned nHeight,
...
CSSD1306Dev::~CSSD1306Dev (void)
{
}

boolean CSSD1306Dev::Initialize (void)
{
...
void CSSD1306Dev::DevClearCursor (void)
{
	// Just clear the display
	Clear(TRUE);
}

void CSSD1306Dev::DevSetCursorMode (boolean bVisible)
{
}

void CSSD1306Dev::DevSetChar (unsigned nPosX, unsigned nPosY, char chChar)
{
	// Draw a non-inverted, non-double width character
	DrawChar (chChar, nPosX, nPosY, FALSE, FALSE);
}

void CSSD1306Dev::DevSetCursor (unsigned nCursorX, unsigned nCursorY)
{
}

void CSSD1306Dev::DevUpdateDisplay (void) {
	WriteFrameBuffer(true);
}

void CSSD1306Dev::WriteCommand(u8 nCommand) const
{
	const u8 Buffer[] = { 0x80, nCommand };
	m_pI2CMaster->Write(m_nAddress, Buffer, sizeof(Buffer));
}

void CSSD1306Dev::WriteFrameBuffer(bool bForceFullUpdate) const
{
	// Reset start line
...
void CSSD1306Dev::SwapFrameBuffers()
{
	// Make other framebuffer current
	m_nCurrentFrameBuffer = (m_nCurrentFrameBuffer + 1) % 2;
}

void CSSD1306Dev::DrawChar(char chChar, u8 nCursorX, u8 nCursorY, bool bInverted, bool bDoubleWidth)
{
	const size_t nRowOffset    = nCursorY * m_nWidth * 2;
	const size_t nColumnOffset = nCursorX * (bDoubleWidth ? 10 : 5) + 4;
...
void CSSD1306Dev::Flip()
{
	WriteFrameBuffer();
	SwapFrameBuffers();
}

void CSSD1306Dev::Print(const char* pText, u8 nCursorX, u8 nCursorY, bool bClearLine, bool bImmediate)
{
	if (bClearLine)
	{
...
void CSSD1306Dev::Clear(bool bImmediate)
{
	u8* pFrameBuffer = m_FrameBuffers[m_nCurrentFrameBuffer].FrameBuffer;
...
void CSSD1306Dev::SetBacklightState(bool bEnabled)
{
